...
export default function ConvertsPage() {
  ...
  // Query for converted prospects only (server-side filtering via index)
  const prospectsData = useQuery(
    api.prospects.getWorkspaceProspects,
    workspaceId ? { workspaceId, status: "converted", limit } : "skip"
  );
  ...
  const convertedProspects = (prospectsData?.prospects ??
  ...
  // Search filter
  const filteredProspects = convertedProspects.filter((p) => {
