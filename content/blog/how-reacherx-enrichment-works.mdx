...
tags: ["engineering", "how reacherx enrichment works"]
---
...
The workflow return type also includes `pending`. For example, it can skip an archived person and return their existing status, falling back to `pending` if none is set. That is separate from the core result above.
