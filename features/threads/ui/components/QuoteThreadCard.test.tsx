...
import { DropdownMenuItem } from "@/shared/ui/components/DropdownMenu";
...
test("quotes without author handles never expose broken profile links", () => {
  const props = cardHandlers({
    id_str: "123",
    full_text: "No author returned",
  });
  const menus = findElements(props.children, ThreadMenu);
  expect(menus[0].props.profileUrl).toBeUndefined();
  const anchors = findElements(props.children, "a");
  expect(
    anchors.some((anchor) =>
      String(anchor.props.href).includes("x.com/undefined")
    )
  ).toBe(false);
  const menu = ThreadMenu({ tweetUrl: "https://x.com/i/status/123" });
  const items = findElements(menu, DropdownMenuItem);
