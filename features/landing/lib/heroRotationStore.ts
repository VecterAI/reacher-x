...
export type RotationSnapshot = { index: number };

/**
 * One shared timer drives every rotating hero element (headline stream and
 * composer placeholder stream) so they always show the aligned pair.
 */
let snapshot: RotationSnapshot = { index: 0 };
...
export const IDLE_ROTATION: RotationSnapshot = { index: 0 };

/**
 * Subscribes to the shared rotation only while enabled, so pages without
 * rotating elements never start the timer. Consumers read the aligned item
