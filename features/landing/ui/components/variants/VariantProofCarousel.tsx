 ...
 * Same mechanism as UseCasesSection on the current landing page.
 */
...
/**
 * Testimonials with the same vertical-scroll-driven horizontal carousel the
 * current landing page uses for use cases.
 */
export function VariantProofCarousel({ tweets }: { tweets: Tweet[] }) {
  ...
  return (
    <ScrollXCarousel className="h-[200vh]">
      <ScrollXCarouselContainer className="top-12 flex h-[calc(100dvh-3rem)] flex-col items-stretch justify-center md:top-14 md:h-[calc(100dvh-3.5rem)]">
        <h2
          id="v2-proof-heading"
          className="font-pixel-square mx-auto mb-12 w-full max-w-4xl px-4 text-center text-4xl leading-tight font-medium text-balance md:mb-16 md:max-w-[1288px] md:text-5xl"
        >
        ...
        <div ref={containerRef}>
          ...
          >
            {tweets.map((tweet) => (
              <div
                key={tweet.id_str}
                className="min-w-[86vw] sm:min-w-[70vw] md:min-w-[40vw] xl:min-w-[32vw]"
              >
                <ThreadCard
                  staticTweet={tweet}
                  size="md"
