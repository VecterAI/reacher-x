...
export function getEditorialDemoPlan(
  ...
  if (!plan || !scenario) return plan;
  if (
    scenario === "workspaces" &&
    (id === "use_case_demo_customers_1" || id === "use_case_demo_customers_2")
  ) {
    const isDaniel = id === "use_case_demo_customers_1";
    const commentDescription = isDaniel
      ? "Reply to Daniel's feedback question"
      : "Reply to Sofia's feedback post";
    return {
      ...plan,
      outreachProgress: plan.outreachProgress
        ? {
            ...plan.outreachProgress,
            activeTask: plan.outreachProgress.activeTask
              ? {
                  ...plan.outreachProgress.activeTask,
        ...
        : undefined,
      rationale: isDaniel
        ? "Daniel wants a simple way to collect feedback from his first twenty users. Ask about his current process and offer to show the app."
        : "Sofia's team keeps feedback in three places. Ask how they review it and offer to show how the app brings it together.",
      tasks: plan.tasks.map((task) =>
        task.type === "comment"
          ? {
              ...
              description: commentDescription,
              content: isDaniel
                ? "Daniel, how are you collecting feedback from those first twenty users? We're building a small app for that. I can show you how it works if you'd like to try it."
                : "Sofia, how does your team decide what to work on when feedback is in three places? We're building an app to bring it together. Would you like to see it?",
            }
          : task.type === "dm"
            ? {
                ...task,
                description: "Offer a walkthrough of the feedback app",
              }
            : task
      ),
    };
  }
  if (id !== "use_case_demo_candidates_1") return plan;
