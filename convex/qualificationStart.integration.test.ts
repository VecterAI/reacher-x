...
test.each([
  ...
  "mismatch",
  "missing",
])("does not start queued work for %s records", async (kind) => {
  const { t, args } = await fixture();
  await t.run(async (ctx) => {
    ...
    else if (kind === "qualified" || kind === "disqualified")
      await ctx.db.patch(args.prospectId, {
        qualificationStatus: kind,
      });
...
test("explicitly starting a legacy unset status claims it as pending without touching other prospects", async () => {
  const { t, args } = await fixture();
  ...
  expect(result.workflowId).not.toBe("");
  expect(await t.run((ctx) => ctx.db.get(args.prospectId))).toMatchObject({
    qualificationStatus: "pending",
    qualificationWorkflowId: result.workflowId,
  });
  const untouched = await t.run((ctx) => ctx.db.get(untouchedId));
  expect(untouched?.qualificationStatus).toBeUndefined();
  expect(untouched?.qualificationWorkflowId).toBeUndefined();
});
