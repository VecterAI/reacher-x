...
describe("controlled existing-workspace requalification", () => {
  test("dry-run processes bounded pages, excludes previews and engaged people, and releases its single-run lock", async () => {
    const { t, workspaceId, prospectId } = await fixture();
    const workflowTestPath = ["@convex-dev/workflow", "test"].join("/");
    const workflowTest = await import(workflowTestPath);
    workflowTest.default.register(t);
    const fingerprint = await t.run(async (ctx) => {
      await ctx.db.patch(workspaceId, { prospectingWorkflowStatus: "paused" });
      const source = (await ctx.db.get(prospectId))!;
      const { _id, _creationTime, ...data } = source;
      for (let index = 0; index < 27; index++) {
        await ctx.db.insert("prospects", {
          ...data,
          externalId: `repair-${index}`,
        });
      }
      await ctx.db.insert("prospects", {
        ...data,
    ...
    try {
      ...
      const workflowId = await t.mutation(
        internal.workflows.workspaceRequalification.startInternal,
        args
      );
      ...
      expect(workspace?.requalificationWorkflowId).toBeUndefined();
      expect(workspace?.lastRequalificationError).toBeUndefined();
      expect(workspace?.lastRequalificationResult).toEqual({
        isDone: true,
        continueCursor: null,
        scanned: 30,
        eligible: 28,
  ...
  test("repair writes require the active run, paused workspace and unchanged intent", async () => {
    const { t, workspaceId, prospectId } = await fixture();
    const fingerprint = await t.run(async (ctx) => {
      await ctx.db.patch(workspaceId, {
        prospectingWorkflowStatus: "paused",
        requalificationWorkflowId: "repair-owner",
      });
      return getLearningTargetingFingerprint((await ctx.db.get(workspaceId))!);
    });
    const args = {
    ...
    await t.run((ctx) =>
      ctx.db.patch(workspaceId, { prospectingWorkflowStatus: "running" })
    );
    await expect(
      t.mutation(
        internal.workflows.workspaceRequalification.resetProspectInternal,
        { ...args, workflowId: "repair-owner" }
      )
    ).rejects.toThrow("Pause and drain");
    await retarget(t, workspaceId);
    await expect(
      t.mutation(
        internal.workflows.workspaceRequalification.resetProspectInternal,
    ...
    expect(
      (await t.run((ctx) => ctx.db.get(prospectId)))?.qualificationStatus
    ).toBe("qualified");
  });
});
...
test("applying repair requires a reviewed dry run within the explicit size bound", async () => {
  const { t, workspaceId } = await fixture();
  const fingerprint = await t.run(async (ctx) => {
    await ctx.db.patch(workspaceId, { prospectingWorkflowStatus: "paused" });
    return getLearningTargetingFingerprint((await ctx.db.get(workspaceId))!);
  ...
  await expect(
    t.mutation(internal.workflows.workspaceRequalification.startInternal, args)
  ).rejects.toThrow("Complete and inspect a dry run");
  await t.run((ctx) =>
    ctx.db.patch(workspaceId, {
      lastRequalificationTargetingFingerprint: fingerprint,
  ...
  await expect(
    t.mutation(internal.workflows.workspaceRequalification.startInternal, args)
  ).rejects.toThrow("exceeds maxProspects");
  expect(
    (await t.run((ctx) => ctx.db.get(workspaceId)))?.requalificationWorkflowId
  ).toBeUndefined();
});

test("large repair dry runs stop at 1000 and expose a continuation cursor", async () => {
  const { t, workspaceId, prospectId } = await fixture();
  const workflowTest = await import(["@convex-dev/workflow", "test"].join("/"));
  workflowTest.default.register(t);
  const fingerprint = await t.run(async (ctx) => {
    await ctx.db.patch(workspaceId, { prospectingWorkflowStatus: "paused" });
    const { _id, _creationTime, ...data } = (await ctx.db.get(prospectId))!;
  ...
  try {
    ...
    await finishScheduledBatches(t, 300);
    const first = (await t.run((ctx) => ctx.db.get(workspaceId)))
      ?.lastRequalificationResult;
    ...
    await finishScheduledBatches(t, 300);
    expect(
      (await t.run((ctx) => ctx.db.get(workspaceId)))?.lastRequalificationResult
    ).toMatchObject({
      scanned: 2,
      eligible: 2,
      completed: 0,
